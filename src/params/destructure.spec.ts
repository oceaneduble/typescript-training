describe('Destructuring', () => {
  ...
  describe('with Arrays', () => {
    ...
    it('can skip indexes in arrays', () => {
      ...
      var getNestedNumbers = getNumbers() 
      var one = getNestedNumbers[0]
      var three = getNestedNumbers[2]
      var six = getNestedNumbers[5]
      expect(one).toBe(1)
			expect(three).toBe(3)
			expect(six).toBe(6)
		})
  ...
  describe('combined together', () => {
      ...
      balance = process(data.balance);
      funds = process(data.funds);
      incomes = process(data.incomes);
      payments = process(data.payments)
